...
В качестве балансера будет использоваться nginx. Так как предполагается использование протокола HTTP/2, выбрана L7 балансировка. Терминацию SSL выполняет nginx. Так как инстансов nginx`а будет несколько, будет использована dns балансировка. Алгоритм балансировки Round Robin.

## Обеспечение отказоустойчивости
Для обеспечения отказоустойчивости каждый сервер БД имеет master-slave репликацию. Каждая группа Backend серверов имеет 1 запасной сервер для обеспечения высокой доступности. 
