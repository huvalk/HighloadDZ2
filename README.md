...
В качестве балансера будет использоваться nginx. Так как предполагается использование протокола HTTP/2, выбрана L7 балансировка. Терминацию SSL выполняет nginx.  Балансировка между nginx'ами на уровне DNS выполняется алгоритмом Round Robin с ручной уборкой неотвечающих ip адресов.
